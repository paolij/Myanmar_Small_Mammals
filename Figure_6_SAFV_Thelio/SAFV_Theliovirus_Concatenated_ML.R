setwd("/Users/julia/Dropbox/MAVIAN_Lab/Manuscripts/Myanmar/Data_Scripts_Figures/Figure_6_SAFV_Theliovirus_Concatenated_ML/")
library("ggtree")
library("phytools")
library("ggplot2")
library("ape")
...
# Load the tree
ml_tree <- read.tree("Figure_6_SAFV_Theliovirus_Concatenated_ML.treefile")
...
q
  ...
  # Load metadata
  metadata <- read.csv('SAFV_Theliovirus_Concatenated_ML.csv', header = TRUE)
...
# Save the plot
ggsave("Figure_6_SAFV_Theliovirus_Concatenated_ML.pdf", width = 65, height = 75, units = "in", dpi = 600, limitsize = FALSE)