setwd("/Users/julia/Dropbox/MAVIAN_Lab/Manuscripts/Myanmar_September/Data_Scripts_Figures/Figure_S3_SAFV_Thelio_P3/")
library("ggtree")
library("phytools")
library("ggplot2")
library("ape")
...
# Load the tree
ml_tree <- read.tree("SAFV_Thelio_P3_fixed.nwk")
...
q <- ggtree(ml_tree, ladderize = TRUE) +
  ...
  geom_tiplab(aes(subset = node %in% c(55), label = label), size = 12, align = TRUE, offset = 0.08, fontface = 'bold', color = "black") +  # Bold node 7
  geom_tiplab(aes(subset = !node %in% c(55), label = label), size = 12, align = TRUE, offset = 0.08, color = "black")  # Regular for others
q
...
# Save the plot
ggsave("SAFV_Thelio_p3.pdf", width = 65, height = 75, units = "in", dpi = 600, limitsize = FALSE)